...
def init_ldm_parameters(encoding_dim,
                        output_dim,
        ...
        Returns:
        ------------
        - A: torch.Tensor, shape: ( dim_x,  dim_x), State transition matrix of LDM
        - C: torch.Tensor, shape: ( dim_a,  dim_x), Observation matrix of LDM
        - W_log_diag: torch.Tensor, shape: ( dim_x, ), Log-diagonal of dynamics noise covariance matrix (W, therefore it is diagonal and PSD)
        - R_log_diag: torch.Tensor, shape: ( dim_a, ), Log-diagonal of observation noise covariance matrix  (R, therefore it is diagonal and PSD)
        - mu_0: torch.Tensor, shape: ( dim_x, ), Dynamic latent factor prediction initial condition (x_{0|-1}) for Kalman filtering
        - Lambda_0: torch.Tensor, shape: ( dim_x,  dim_x), Dynamic latent factor estimate error covariance initial condition (P_{0|-1}) for Kalman filtering
        ...
        * We learn the log-diagonal of matrix W and R to satisfy the PSD constraint for cov matrices. Diagnoal W and R are used for the stability of learning 
        ...
        kernel_initializer_fn = get_kernel_initializer_function( ldm_kernel_initializer)
        A = kernel_initializer_fn( init_A_scale * torch.eye( encoding_dim, dtype=torch.float32)) 
        C = kernel_initializer_fn( init_C_scale * torch.randn( output_dim,  encoding_dim, dtype=torch.float32)) 

        W_log_diag = torch.log(kernel_initializer_fn(torch.diag( init_W_scale * torch.eye( encoding_dim, dtype=torch.float32))))
        R_log_diag = torch.log(kernel_initializer_fn(torch.diag( init_R_scale * torch.eye( output_dim, dtype=torch.float32))))
        
        mu_0 = kernel_initializer_fn(torch.zeros( encoding_dim, dtype=torch.float32))
        Lambda_0 = kernel_initializer_fn( init_cov * torch.eye( encoding_dim, dtype=torch.float32))
...
class LDM(nn.Module):
    ...
    def get_k_step_postnet(self, k):
        '''
        Compute the k-step ahead postnet matrix W_k

        Parameters: 
        ------------
        - k: int, the number of steps in the future

        Returns: 
        ------------
        - W_k: torch.Tensor, shape: (self.dim_a, self.dim_x), the postnet matrix
        '''
        W_k = self.C @ torch.linalg.matrix_power(self.A, k)
        return W_k
